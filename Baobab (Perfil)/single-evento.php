		...
		<article id="publicacion">
			...
			<ul class="info">
				<li>
					...
					<h4><?php _e("Lugar", "Baobab"); ?></h4><br>
					<p><?php echo get_post_meta($post->ID, 'Lugar', true); ?></p>
					<img src="<?php bloginfo('stylesheet_directory'); ?>/images/single_fecha.png"/>
					<h4><?php _e("Fecha", "Baobab"); ?></h4><br>
					<p><?php the_modified_date('l j, F Y') ?></p>
				</li>
				<li>
					<h4><?php _e("Asistentes", "Baobab"); ?></h4><br>
                    ...
                    if(!is_user_participating($post->ID,$current_user->ID) && is_user_logged_in()) {?>
                        ...
                        <form id="formulario-proyecto-unirse" method="post" action="<?php bloginfo('url'); ?>/index.php">
                           ...
                           <input type="submit" class="button color_green" value='<?php _e("Asistiré","Baobab");?>' />
                        </form>
                        <!-- fin interfaz -->
                    <?php } else { ?>
                        ...
                        <!-- fin interfaz -->
                    <?php } ?>
					<?php if ( is_user_logged_in() ) { echo ''; } else { 	echo  _e("Para participar debe ", "Baobab"); echo "<a href='".site_url()."/wp-login.php'>"; echo _e("Inicia Sesión</a>", "Baobab");}?>
				</li>
			</ul>
	</div>
