...
<div id="pagewrap">
	...
	<div class="profile-info">
		...
		<h2><?php $curauth = (isset($_GET['author_name'])) ? get_user_by('slug', $author_name) : get_userdata(intval($author)); ?><?php echo $curauth->nickname; ?>
			...
			<?php if ($id = $curauth->ID == $current_user->ID){ ?><a href="<?php echo site_url(); ?>/wp-admin/profile.php"><?php _e("Editar Perfil", "Baobab");?></a><?php } ?></span>
		</h2>
		...
		<a href="http://twitter.com/<?php $id = $curauth->ID; $key = 'twitter'; echo esc_attr( get_the_author_meta( $key, $id) );  ?>"><?php $id = $curauth->ID; $key = 'twitter'; echo esc_attr( get_the_author_meta( $key, $id) );  ?></a>

	</div><!-- profile-info -->

	<h4 class="profile-title"><?php _e("Publicaciones de", "Baobab"); ?> <?php echo $curauth->nickname; ?>:</h4>
