				...
				<li>
                    ...
                    if(!is_user_participating($post->ID,$current_user->ID) && is_user_logged_in()) {?>
                        ...
                        <form id="formulario-proyecto-unirse" method="post" action="<?php bloginfo('url'); ?>/index.php">
                           ...
                           <input type="submit" class="button color_green" value="Únete" />
                        </form>
                        <!-- fin interfaz -->
                    <?php } else { ?>
...
if ( is_user_logged_in() ) {
	...
	echo  "Para participar debes <a href='".site_url()."/wp-login.php'>Iniciar Sesión</a>"; 
}
?>
				</li>
			</ul>
		...
		<aside id="archive-list">						
			<?php if ( is_user_logged_in() ) { echo "<a href='".get_permalink_by_postname('enviar-proyecto')."' class='button color_blue'>Crear Proyecto</a>"; } ?>			
