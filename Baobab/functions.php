...
function baobab_setup() {
	...
	load_theme_textdomain( 'baobab', get_template_directory(). '/languages' );
...
/* Comentarios */
function baobab_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php _e( "Pingback:", "Baobab"); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( "Editar", "Baobab" ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
	?>
		...
		<div id="comment-<?php comment_ID(); ?>" class="comment">
				<div class="comment-author vcard">
					<?php
						...
						echo get_avatar( $comment->user_id, $avatar_size );?>




<?php if ($comment->user_id) {?><?php echo '<a href="'. get_author_posts_url($comment->user_id) .'">';?> <?php $user=get_userdata($comment->user_id); echo $user->user_firstname; echo ' ';  echo $user->user_lastname;?><?php echo '</a>';}?>		 
					<?php
						printf( __( '%1$s, %2$s <span class="says"></span>'),
							...
							sprintf( '<span="%1$s"><time pubdate datetime="%2$s">%3$s</time></span>', /*en lugar de span era un a href*/
								...
								sprintf( __( '%1$s at %2$s', "Baobab"), get_comment_date(), get_comment_time() )
							)
						);					
					?>
				...
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em class="comment-awaiting-moderation"><?php _e( "Tu comentario está esperando ser aprobado.", "Baobab"); ?></em>
					<br />
				<?php endif; ?>
			...
			<div class="reply">
			 <?php if(is_user_logged_in()){ ?><?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( "Responder", "Baobab"), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?><?php } ?>
			</div><!-- .reply -->
...
if ( isset($_POST['_proyecto_nonce']) ) {
	if ( wp_verify_nonce($_POST['_proyecto_nonce'], 'enviar_proyecto') ) {
		if ( isset($_POST['edit']) == 'true' ) {
			...
			$insert = wp_update_post(array(
				...
				'post_excerpt' => isset($_POST['proyecto']['imagen']),
				'post_type' => 'proyecto',
				'post_status' => 'publish'
			));
		} else {
...
if ( isset($_POST['_proyecto_unirse']) ) {
	...
	if ( wp_verify_nonce($_POST['_idea_nonce'], 'enviar_idea') ) {
		if ( isset($_POST['edit']) == 'true' ) {
			...
			$insert = wp_update_post(array(
				...
				'post_excerpt' => isset($_POST['idea']['imagen']),
				'post_type' => 'idea',
				'post_status' => 'publish'
			));
		} else {
	...
	if ( wp_verify_nonce($_POST['_mercado_nonce'], 'enviar_mercado') ) {
		if ( isset($_POST['edit']) == 'true' ) {
			...
			$insert = wp_update_post(array(
				...
				'post_excerpt' => isset($_POST['mercado']['imagen']),
				'post_type' => 'mercado',
				'post_status' => 'publish'
			));
		} else {
...
if ( isset($_POST['edit']) ) {
		if ( $_POST['edit'] == 'true' ) {
			...
			$insert = wp_update_post(array(
				...
				'post_excerpt' => isset($_POST['debate']['imagen']),
				'post_type' => 'debate',
				'post_status' => 'publish'
			));
		}
		...
		if ( is_wp_error($insert) ) {
			...
			if ( isset($_POST['debate']['pregunta'] ) ) {
...
if ( isset($_POST['_debate_realizar_voto']) ) {
	...
	if ( wp_verify_nonce($_POST['_evento_nonce'], 'enviar_evento') ) {
		...
		if ( isset($_POST['edit']) == 'true' ) {
			...
			$insert = wp_update_post(array(
				...
				'post_excerpt' => isset($_POST['evento']['imagen']),
				'post_type' => 'evento',
				'post_status' => 'publish'
			));
		} else {
...
function listar_noticias(){
	...
	if ( $noticias->have_posts() ) : $i = 0;
		echo '<ul><div class="circle"></div>';
			...
			echo ''.the_author_meta('first_name').'</a>';
			echo ', ';
 ...
 **/
function baobab_category_checkbox( $objeto, $edit = false ){
	if ( $edit ) {
	...
	if ( $terms ) {
		...
		foreach ( $terms as $term ) {
			$exits = array($term->term_id) ? '' : ''; /* $exits = in_array($term->term_id, $post_terms) ? ' checked="checked" ' : ''; */
...
if ( !function_exists('fb_addgravatar') ) {
	function fb_addgravatar( $avatar_defaults ) {
		$myavatar = get_template_directory_uri('template_directory') . '/images/avatar.png';
...
/* Página: Página de Logueo -> iniciar-sesion.php */
if ( isset($_GET['activated']) ) {
if (isset($_GET['activated']) && is_admin())
	$pages = array(
	...
	$id = wp_insert_post( $post, $wp_error);
		$template = strtolower("iniciar-sesion.php");
		$update = update_post_meta($id, '_wp_page_template', $template);
	}
}
...
function save_extra_profile_fields( $user_id ) {
	...
	update_user_meta( $user_id, 'agree', $_POST['agree'] );
...
function custom_avatars($avatar, $id_or_email, $size){
  ...
  if(is_user_logged_in()){
    ...
    $image_url = get_user_meta($id_or_email, 'custom_avatar', true); 
  
    if($avatar !== false) /* $user_avatar */
...
/* Esta función me permite acceder con el nombre de Usuario o su E-mail */
...
/* Límite de Palabras (Páginas de archivo)*/
