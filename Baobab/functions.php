...
add_image_size('noticia', 200, 0, true);
add_image_size('noticia-thumbnail', 440, 0, true);
...
/* Noticias */
function registrar_tipo_noticia(){
...
add_action('init', 'registrar_tipo_noticia');
add_action('init', 'recibir_noticia');
...
/* Noticia */
function recibir_noticia(){
	if ( wp_verify_nonce($_POST['_noticia_nonce'], 'enviar_noticia') ) {
		if ( $_POST['edit'] == 'true' ) {
			$referer = $_POST['_wp_http_referer'];
			$url = parse_url($referer);
			parse_str($url['query'], $query);
			$insert = wp_update_post(array(
				'ID' => $query['id'],
				'post_title' => $_POST['noticia']['nombre'],
				'post_content' => $_POST['noticia']['descripcion'],
				'post_excerpt' => $_POST['noticia']['imagen'],
				'post_type' => 'noticia',
				'post_status' => 'publish'
			));
		} else {
		...
		if ( is_wp_error($insert) ) {
			...
			foreach( $_POST['noticia']['categorias'] as $categoria ){
				wp_set_post_terms( $insert, (int)$categoria, 'category', true );
			}
			wp_redirect( get_permalink($insert) );
			exit;
