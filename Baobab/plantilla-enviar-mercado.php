...
<div class="wrap">
	<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
		<?php // echo '<pre>'. print_r($post, true) .'</pre>'; ?>
			...
			<h2 class="titulo-post"><?php _e("Crear Mercado", "Baobab"); ?></h2>
			<?php endif; ?>
			<?php
			...
			<form id="formulario-mercado" method="post" action="<?php echo home_url(); ?>/index.php" enctype="multipart/form-data">
				<fieldset>
					...
					<?php if (is_user_logged_in()){ ?>
						...
						<h4><?php _e("Título", "Baobab"); ?></h4>
							...
							echo '<input class="datos" name="mercado[nombre]" type="text"/>';
							}else{ 
							echo '<input class="datos" name="mercado[nombre]" type="text" value="'.esc_attr($mercado->post_title).'"/>';
						}?>

						<h4><?php _e("Descripción", "Baobab"); ?></h4>						
						<?php if (!isset($_GET['id'])){ 
							echo '<textarea name="mercado[descripcion]" class="text-input descripcion" style="width:100%">';
							echo '</textarea>';
							}else{ 
							echo '<textarea name="mercado[descripcion]" class="text-input descripcion" style="width:100%">';
							echo esc_textarea( $mercado->post_content );
							echo '</textarea>';
						}?>
						...
						<?php
						if ( isset($_GET['id']) ) {
							$valor = get_post_meta($_GET['id'], 'Valor', true);
							echo '<input type="text" class="datos" name="mercado[valor]" value="'. esc_attr($valor) .'" />';
							}else{
							echo '<input type="text" class="datos" name="mercado[valor]" />';
							}
						?>

						<h4><?php _e("Condición", "Baobab"); ?></h4>
						<select name="mercado[condicion]">
  							<option><?php _e("Vendo", "Baobab"); ?></option>
  							<option><?php _e("Compro", "Baobab"); ?></option>
  							<option><?php _e("Transacción Finalizada", "Baobab"); ?></option>
						</select>
						...
						<?php baobab_category_checkbox('mercado', $mercado); ?>
...
if ( is_user_logged_in() ) {
	...
	echo _e("Para publicar un Producto debes", "Baobab");
