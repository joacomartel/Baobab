...
if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'log-in' ) :
	...
	global $error;
//	$login = wp_signon( $_POST['user-name'], $_POST['password'] );
	$login = wp_signon( array( 'user_login' => $_POST['user-name'], 'user_password' => $_POST['password'], 'remember' => isset($_POST['remember-me'] )), false );
...
<!-- Formulario de Logueo -->
	...
	<?php if ( is_user_logged_in() ) : ?>
		...
		<?php elseif ( isset($login->ID) ) : ?><?php $login = get_userdata( $login->ID ); ?><META HTTP-EQUIV="REFRESH" CONTENT="0;URL=<?php echo site_url(); ?>">
	...
	<?php else : // Not logged in ?>
		...
		<form action="<?php the_permalink(); ?>" method="post" class="sign-in">
				...
				<input type="text" name="user-name" id="user-name" class="datos"/>

				<h4><?php _e("Contraseña", "Baobab"); ?></h4>
				<input type="password" name="password" id="password" class="datos"/>

				<input type="submit" name="submit" class="btn blue" value="<?php _e("Inicia Sesión","Baobab"); ?>" />
